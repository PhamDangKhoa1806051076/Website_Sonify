...
function EntryView({ username, setUsername, onContinue, onSwitchToSignup, error }: {
    ...
    return (
        <motion.div key="entry" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <OutlineButton onClick={() => { window.location.href = '/api/auth/github'; }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink: 0, color: '#e6edf3' }}>
                ...
                <span style={{ flex: 1, textAlign: 'left' }}>Tiếp tục bằng GitHub</span>
            </OutlineButton>
